...
**ZhEnTextFormatter** 是一个轻量级的 JavaScript 插件，旨在优化中英文混排文本的排版。它通过在中英文字符之间自动插入不可见的断行符号，改善文本的可读性。该插件特别适合处理包含中日韩（CJK）字符的网页内容，使混排文本的显示更加美观。

## 特性

- **自动检测**：自动检测中英文字符的切换，智能插入断行符号。
- **支持多种字符**：支持包括中文、日文假名、韩文谚文在内的多种 CJK 字符。
